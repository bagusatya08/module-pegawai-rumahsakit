...
var checker_judul = false;
var checker_konten = false;

judul.addEventListener("keyup", function (e) {
    if (judul.value === "") {
        // tampilkan error dan tambahkan class error
        setError(judul, "judul tidak boleh kosong");
        checker_judul = false;
    } else if (cekSimbol(judul.value)) {
        // tampilkan error dan tambahkan class error
        setError(judul, "judul tidak boleh mengandung simbol");
        checker_judul = false;
    } else if (judul.value.length < 2) {
        // tampilkan error dan tambahkan class error
        setError(judul, "judul harus lebih dari 1 huruf");
        checker_judul = false;
    } else if (!cekAlamat(judul.value)) {
        // tampilkan error dan tambahkan class error
        setError(judul, "judul tidak boleh diawali atau diakhiri spasi");
        checker_judul = false;
    } else {
...
konten.addEventListener("keyup", function (e) {
    if (konten.value === "") {
        // tampilkan error dan tambahkan class error
        setError(konten, "konten tidak boleh kosong");
        checker_konten = false;
    } else if (konten.value.length < 2) {
        // tampilkan error dan tambahkan class error
        setError(konten, "konten harus lebih dari 1 huruf");
        checker_konten = false;
    } else {
        // tambahkan class success
        setSuccess(konten);
...
form.addEventListener("submit", (e) => {
    if (checker_judul == false || checker_konten == false) {
